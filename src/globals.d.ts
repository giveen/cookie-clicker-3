...
declare global {
	...
	interface Window {
		 ...
		 * and the Collect button's payoff-feedback entry point the prompt's
		 * inline handler calls through this global); used by ?qa=dailycrumb. */
		__cc3DailyCrumb?: {
		...
		__cc3Transcendence?: {
			...
			DOCTRINE_MOONS: readonly { id: string; planetId: number; name: string; desc: string; cost: number; icon: number[]; orbitRadius: number; orbitPeriod: number; baseAngle: number }[];
			...
			purchaseMoon: (moonId: string) => boolean;
			buyMoon: (moonId: string) => void;
			respec: () => void;
			respecAndRedraw: () => void;
			checkMilestones: () => void;
			checkAchievements: () => void;
			doctrineHas: (id: number) => boolean;
			moonHas: (id: string) => boolean;
