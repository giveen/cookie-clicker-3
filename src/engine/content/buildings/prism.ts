...
export function declarePrism(Game: EngineGame) {
		...
		var prismObj=Game.Objects['Prism'];
		var prismCellW=64;
		var prismCellH=80;
		var prismSheetCols=3;
		var prismSheetRows=2;
		prismObj.draw=function(this: Building)
		{
			...
			var cellSrcW=sheet.naturalWidth?Math.round(sheet.naturalWidth/prismSheetCols):prismCellW;
			var cellSrcH=sheet.naturalHeight?Math.round(sheet.naturalHeight/prismSheetRows):prismCellH;
			var scale=Math.min(1,STACK_TARGET_H/prismCellH);
			var drawW=prismCellW*scale;
			var drawH=prismCellH*scale;
			...
			if (i!=iT)
			{
				while (i<iT)
				{
					...
					var sx=(i%prismSheetCols)*cellSrcW;
					var sy=(Math.floor(i/prismSheetCols)%prismSheetRows)*cellSrcH;
