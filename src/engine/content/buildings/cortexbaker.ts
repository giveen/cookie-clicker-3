...
export function declareCortexBaker(Game: EngineGame) {
		...
		var cbObj=Game.Objects['Cortex baker'];
		var cbCellW=64;
		var cbCellH=80;
		var cbSheetCols=3;
		var cbSheetRows=2;
		cbObj.draw=function(this: Building)
		{
			...
			var cellSrcW=sheet.naturalWidth?Math.round(sheet.naturalWidth/cbSheetCols):cbCellW;
			var cellSrcH=sheet.naturalHeight?Math.round(sheet.naturalHeight/cbSheetRows):cbCellH;
			var scale=Math.min(1,STACK_TARGET_H/cbCellH);
			var drawW=cbCellW*scale;
			var drawH=cbCellH*scale;
			...
			if (i!=iT)
			{
				while (i<iT)
				{
					...
					var sx=(i%cbSheetCols)*cellSrcW;
					var sy=(Math.floor(i/cbSheetCols)%cbSheetRows)*cellSrcH;
