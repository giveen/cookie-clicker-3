...
export function declarePortal(Game: EngineGame) {
		...
		var portObj=Game.Objects['Portal'];
		var portCellW=64;
		var portCellH=80;
		var portSheetCols=3;
		var portSheetRows=2;
		portObj.draw=function(this: Building)
		{
			...
			var cellSrcW=sheet.naturalWidth?Math.round(sheet.naturalWidth/portSheetCols):portCellW;
			var cellSrcH=sheet.naturalHeight?Math.round(sheet.naturalHeight/portSheetRows):portCellH;
			var scale=Math.min(1,STACK_TARGET_H/portCellH);
			var drawW=portCellW*scale;
			var drawH=portCellH*scale;
			...
			if (i!=iT)
			{
				while (i<iT)
				{
					...
					var sx=(i%portSheetCols)*cellSrcW;
					var sy=(Math.floor(i/portSheetCols)%portSheetRows)*cellSrcH;
