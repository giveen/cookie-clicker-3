...
export function declareAntimatterCondenser(Game: EngineGame) {
		...
		var acObj=Game.Objects['Antimatter condenser'];
		var acCellW=64;
		var acCellH=80;
		var acSheetCols=3;
		var acSheetRows=2;
		acObj.draw=function(this: Building)
		{
			...
			var cellSrcW=sheet.naturalWidth?Math.round(sheet.naturalWidth/acSheetCols):acCellW;
			var cellSrcH=sheet.naturalHeight?Math.round(sheet.naturalHeight/acSheetRows):acCellH;
			var scale=Math.min(1,STACK_TARGET_H/acCellH);
			var drawW=acCellW*scale;
			var drawH=acCellH*scale;
			...
			if (i!=iT)
			{
				while (i<iT)
				{
					...
					var sx=(i%acSheetCols)*cellSrcW;
					var sy=(Math.floor(i/acSheetCols)%acSheetRows)*cellSrcH;
