...
export function declareChancemaker(Game: EngineGame) {
		...
		var cmObj=Game.Objects['Chancemaker'];
		var cmCellW=64;
		var cmCellH=80;
		var cmSheetCols=3;
		var cmSheetRows=2;
		cmObj.draw=function(this: Building)
		{
			...
			var cellSrcW=sheet.naturalWidth?Math.round(sheet.naturalWidth/cmSheetCols):cmCellW;
			var cellSrcH=sheet.naturalHeight?Math.round(sheet.naturalHeight/cmSheetRows):cmCellH;
			var scale=Math.min(1,STACK_TARGET_H/cmCellH);
			var drawW=cmCellW*scale;
			var drawH=cmCellH*scale;
			...
			if (i!=iT)
			{
				while (i<iT)
				{
					...
					var sx=(i%cmSheetCols)*cellSrcW;
					var sy=(Math.floor(i/cmSheetCols)%cmSheetRows)*cellSrcH;
