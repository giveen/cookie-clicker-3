...
export function declareTemple(Game: EngineGame) {
		...
		// CC3: same staggered, overlapping vertical-stack treatment as Farms, Mines, Factories, and Banks,
		// on the shared STACK layout. Back rows are shaded for atmospheric depth.
		var templeObj=Game.Objects['Temple'];
		var templeCellW=64;
		var templeCellH=80;
		var templeSheetCols=3;
		var templeSheetRows=2;
		templeObj.draw=function(this: Building)
		{
			...
			var cellSrcW=sheet.naturalWidth?Math.round(sheet.naturalWidth/templeSheetCols):templeCellW;
			var cellSrcH=sheet.naturalHeight?Math.round(sheet.naturalHeight/templeSheetRows):templeCellH;
			var scale=Math.min(1,STACK_TARGET_H/templeCellH);
			var drawW=templeCellW*scale;
			var drawH=templeCellH*scale;
			...
			if (i!=iT)
			{
				while (i<iT)
				{
					...
					var sx=(i%templeSheetCols)*cellSrcW;
					var sy=(Math.floor(i/templeSheetCols)%templeSheetRows)*cellSrcH;
