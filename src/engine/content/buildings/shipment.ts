...
export function declareShipment(Game: EngineGame) {
		...
		var shipObj=Game.Objects['Shipment'];
		var shipCellW=64;
		var shipCellH=80;
		var shipSheetCols=3;
		var shipSheetRows=2;
		shipObj.draw=function(this: Building)
		{
			...
			var cellSrcW=sheet.naturalWidth?Math.round(sheet.naturalWidth/shipSheetCols):shipCellW;
			var cellSrcH=sheet.naturalHeight?Math.round(sheet.naturalHeight/shipSheetRows):shipCellH;
			var scale=Math.min(1,STACK_TARGET_H/shipCellH);
			var drawW=shipCellW*scale;
			var drawH=shipCellH*scale;
			...
			if (i!=iT)
			{
				while (i<iT)
				{
					...
					var sx=(i%shipSheetCols)*cellSrcW;
					var sy=(Math.floor(i/shipSheetCols)%shipSheetRows)*cellSrcH;
