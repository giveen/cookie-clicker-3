 ...
 * Reliquary, challenge rewards, Deep Delve, the mortal staircase, the
 * cookie-cow unlock pair "How to milk a cookie cow" / "A certain
 * cow" — systems/cow.ts, and the 17-upgrade Heavenly Pantry branch
 * off 'Box of brand biscuits', icons from the generated
 * public/img/foodIcons.png atlas — see scripts/extract-food-icons.mjs).
...
export function declareVanillaUpgrades(Game: EngineGame) {
		...
		new Game.Upgrade('Second helping',getStrCookieProductionMultiplierPlus(3)+'<q>There\'s room on the plate. There\'s always room on the plate.</q>',8,[25,6]);Game.last.pool='prestige';Game.last.parents=['Heavenly cookies'];Game.last.power=3;Game.last.pseudoCookie=true;//CC3 icon fix: [34,6] is an empty sprite-sheet cell — the pseudo-cookie trio uses the golden-cookie icon family [24,6]/[25,6]/[26,6]
		...
		new Game.Upgrade('Angelic recipe',getStrCookieProductionMultiplierPlus(2)+'<q>Annotated in at least three celestial languages. The margins are just doodles of cookies.</q>',25,[26,6]);Game.last.pool='prestige';Game.last.parents=['Angels'];Game.last.power=2;Game.last.pseudoCookie=true;//CC3 icon fix: [19,11] is an empty sprite-sheet cell — see the note on Second helping
		...
		// CC3: The Heavenly Pantry — a 17-upgrade prestige branch off
		// 'Box of brand biscuits' (extending the vanilla Heavenly cookies →
		// biscuits/macarons chain), so players with hundreds of heavenly chips
		// have a fresh, food-themed tree to climb. Icons come from a dedicated
		// atlas (public/img/foodIcons.png, 48px grid, generated by
		// scripts/extract-food-icons.mjs from dotoridev's "Free - 500+ Pixel
		// Food Icons" pack — CC BY 4.0), referenced via the custom-sheet icon
		// form [col,row,'img/foodIcons.png',48]. Declared at the very end of
		// the upgrade list (after the cookie cow) so existing upgrade ids stay
		// save-stable. Effects reuse the mortal-staircase hooks: economy.ts
		// (click power), ui/store.ts + core/upgrade.ts (discounts),
		// shimmerTypes.ts (golden frequency/duration/lifespan), save.ts
		// (offline CpS), calculateGains.ts (per-pantry CpS capstone).
		order=26000;
		new Game.Upgrade('Pantry key',loc("Unlocks the <b>Heavenly Pantry</b>.")+'<q>Behind the pearly gates, past the lemonade stand: a door marked "snacks". It has never once been locked.</q>',50,[0,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Box of brand biscuits'];
		new Game.Upgrade('Shelf-stable milk',getStrCookieProductionMultiplierPlus(2)+'<q>Does the pantry even get hungry? No. But its residents do.</q>',100,[1,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Pantry key'];Game.last.power=2;Game.last.pseudoCookie=true;
		new Game.Upgrade('Honeypot on a chain',loc("Clicking is <b>%1% stronger</b>.",5)+'<q>Not for the bears. The bears know where it is.</q>',150,[2,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Pantry key'];
		new Game.Upgrade('Jars of preserved starlight',loc("Golden cookies appear <b>%1%</b> more often.",3)+'<q>Keep away from direct sunlight. It gets jealous.</q>',200,[3,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Pantry key'];
		new Game.Upgrade('Bottomless honey jar',loc("You start with <b>%1</b>.",loc("%1 cursor",5))+'<q>Every ascent, someone tips it out and it fills right back up.</q>',250,[4,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Pantry key'];
		new Game.Upgrade('Tinned peaches',loc("Golden cookie effects last <b>%1% longer</b>.",4)+'<q>Sweetness, suspended. The syrup is the best part and everyone knows it.</q>',300,[5,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Shelf-stable milk'];
		new Game.Upgrade('Cosmic jam jars',loc("Golden cookies stay <b>%1%</b> longer.",4)+'<q>The label says strawberry. The label is being polite.</q>',350,[6,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Shelf-stable milk'];
		new Game.Upgrade('Waffle iron of destiny',loc("Clicking is <b>%1% stronger</b>.",7)+'<q>It has opinions about batter. They are correct opinions.</q>',400,[7,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Honeypot on a chain'];
		new Game.Upgrade('Sourdough starter, vintage',loc("All buildings are <b>%1% cheaper</b>.",1)+'<q>Older than most civilizations. Feeds weekly. Judge it at your peril.</q>',450,[8,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Pantry key'];
		new Game.Upgrade('Dimensional spice rack',loc("All upgrades are <b>%1% cheaper</b>.",1)+'<q>Every spice is from somewhere slightly else.</q>',500,[9,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Pantry key'];
		new Game.Upgrade('Everlasting gravy boat',loc("You gain another <b>+%1%</b> of your regular CpS while the game is closed.",2)+' <small>('+loc("Must own the %1 upgrade.",getUpgradeName("Twin Gates of Transcendence"))+')</small><q>It refills out of spite.</q>',600,[10,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Tinned peaches','Cosmic jam jars'];
		new Game.Upgrade('Casserole of the congregation',getStrCookieProductionMultiplierPlus(3)+'<q>Every church potluck in history has featured this exact casserole. Now you know where it goes afterward.</q>',700,[11,0,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Tinned peaches','Cosmic jam jars'];Game.last.power=3;Game.last.pseudoCookie=true;
		new Game.Upgrade('Thousand-year egg',loc("Golden cookies appear <b>%1%</b> more often.",5)+'<q>It has been ready for centuries. It was waiting for you.</q>',800,[0,1,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Casserole of the congregation'];
		new Game.Upgrade('Comet cotton candy',loc("Golden cookie effects last <b>%1% longer</b>.",6)+'<q>Spun from a passing comet\'s tail. Slightly gritty. Cosmically sweet.</q>',900,[1,1,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Casserole of the congregation'];
		new Game.Upgrade('Grandma\'s recipe box',loc("Clicking is <b>%1% stronger</b>.",10)+'<q>Every card is annotated. Half the annotations are "more butter".</q>',1000,[2,1,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Waffle iron of destiny'];
		new Game.Upgrade('Bottomless pantry',loc("Cookie production multiplier <b>+%1%</b> per Heavenly Pantry upgrade owned.",1)+'<q>The shelves go back further than the building does.</q>',2500,[3,1,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Everlasting gravy boat','Casserole of the congregation','Thousand-year egg','Comet cotton candy','Grandma\'s recipe box','Sourdough starter, vintage','Dimensional spice rack'];Game.last.power=function(){return Game.PantryUpgradesOwned();};Game.last.pseudoCookie=true;
		new Game.Upgrade('The Eternal Feast',getStrCookieProductionMultiplierPlus(10)+'<q>Nobody has ever reached the end of the table. Some say the table grows. Some say you simply never want to stop.</q>',10000,[4,1,'img/foodIcons.png',48]);Game.last.pantry=1;Game.last.pool='prestige';Game.last.parents=['Bottomless pantry'];Game.last.power=10;Game.last.pseudoCookie=true;
